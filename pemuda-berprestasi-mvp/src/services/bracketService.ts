...
static generateByeOrder(bracketSize: number): number[] {
  ...
  const addBalanced = (start: number, end: number) => {
    ...
    // Tambah posisi bawah (jika ada)
    if (start + 1 < end) {
      positions.push(end - 1);
    }
    
    // Rekursif untuk posisi tengah
    addBalanced(start + 1, end - 1);
  };
  
  addBalanced(0, bracketSize);
  
  console.log(`🎯 Generated BYE order for bracket size ${bracketSize}:`, positions);
  return positions;
}
...
static async generatePrestasiBracket(
  ...
  for (let i = 0; i < round1MatchCount; i++) {
    ...
    matches.push({
      ...
      round: 1,
      position: i,
      participant1: slotA.isBye ? null : slotA.participant,
      participant2: slotB.isBye ? null : slotB.participant,
      status: (slotA.isBye || slotB.isBye) ? 'bye' : 'pending',
      scoreA: 0,
      scoreB: 0
    });

    const displayA = slotA.isBye ? 'BYE' : (slotA.participant?.name || 'NULL');
    const displayB = slotB.isBye ? 'BYE' : (slotB.participant?.name || 'NULL');
    console.log(`  Match ${match.id_match}: ${displayA} vs ${displayB}`);
  }
