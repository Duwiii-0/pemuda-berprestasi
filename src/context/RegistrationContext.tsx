...
type RegistrationContextType = {
  ...
  validateRegistration: (kompetisiId: number) => ValidationResult;
  registerAtlet: (kompetisiId: number) => Promise<RegistrationResponse | null>;
...
const getRegistrationsByKompetisi = useCallback(async (kompetisiId: number): Promise<RegistrationType[]> => {
  ...
  const validateRegistration = useCallback((kompetisiId: number): ValidationResult => {
    const errors: string[] = [];
    const warnings: string[] = [];

    // Basic validations (same as before)
    if (!kompetisiId) {
      errors.push("ID Kompetisi tidak valid");
    }
    ...
    if (!formData.kelasKejuaraanId) {
      errors.push("Kelas kejuaraan tidak ditemukan");
    }

    // Check for duplicate registrations
    const selectedAthletes = getSelectedAthletes();
    const duplicateAthletes = selectedAthletes.filter(athlete => 
      existingRegistrations.some(reg => 
        reg.atlitId === athlete.id || reg.atlitId2 === athlete.id
      )
    );

    if (duplicateAthletes.length > 0) {
      const names = duplicateAthletes.map(a => a.nama).join(', ');
      errors.push(`Atlet berikut sudah terdaftar dalam kompetisi ini: ${names}`);
    }

    // Style-specific validations (same as before)
    ...
    if (formData.styleType === "POOMSAE") {
      ...
      if (isPoomsaeTeam()) {
        ...
        if (formData.selectedAtlit && formData.selectedAtlit2 && 
  ...
  }, [formData, isPoomsaeTeam, getSelectedAthletes, existingRegistrations]);

  // ✅ REGISTRATION: Main registration function
  const registerAtlet = useCallback(async (kompetisiId: number): Promise<RegistrationResponse | null> => {
    try {
      setIsLoading(true);
      ...
      const { selectedAtlit, selectedAtlit2, kelasKejuaraanId } = formData;
      
      if (!selectedAtlit || !kelasKejuaraanId) {
        throw new Error("Data registrasi tidak lengkap");
      }

      // Prepare registration payload
      const payload: RegistrationPayload = {
        atlitId: Number(selectedAtlit.value),
        kelasKejuaraanId: kelasKejuaraanId,
      };

      // Add second athlete for team/pair
      if (requiresTwoAthletes() && selectedAtlit2) {
        payload.atlitId2 = Number(selectedAtlit2.value);
      }
      ...
      if (!response) {
        throw new Error("Gagal mendaftarkan atlet");
      }
  ...
  }, [formData, validateRegistration, requiresTwoAthletes, resetForm]);
