  ...
  const UnifiedRegistration = ({
    ...
    useEffect(() => {
      if (formData.selectedAge && formData.selectedGender) {
        fetchWeightOptions(Number(formData.selectedAge.value), formData.selectedGender.value);
      }
    }, [formData.selectedAge, formData.selectedGender]);

    useEffect(() => {
  // cuma fetch kalau semua data kategori lengkap
  if (formData.categoryType && formData.selectedGender) {
    // nanti id kelas kejuaraan bisa ditentukan dari kombinasi style, kategori, age, weight
    // sementara hardcode dulu id contoh: 1
    const idKelasKejuaraan = 1; 
    fetchEligibleAtlits(idKelasKejuaraan);
  }
}, [formData.categoryType, formData.selectedGender, formData.selectedAge, formData.selectedWeight]);
    ...
    const atlitOptions: OptionType[] = availableAtlits.map((a) => ({
      value: a.id.toString(),
      label: `${a.nama} - ${a.provinsi}`,
    }));
