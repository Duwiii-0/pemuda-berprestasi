...
const GeneralButton: React.FC<GeneralButtonProps> = ({ label, type, to, onClick, className, disabled }) => {
  if (type === "link" && to) {
    ...
    className={`py-6 md:py-0 text-center btn px-6 rounded-2xl font-plex flex justify-center items-center cursor-pointer ${className}`}>{label}</Link>; // Bisa diganti Link kalau full page routing
  }
  ...
  if (type === "scroll" && to) {
    ...
    return (
      <button
        className={`btn px-6 rounded-2xl font-plex flex justify-center items-center cursor-pointer ${className} `} onClick={onClick}
        disabled={disabled}
      >
        {label}
      </button>
